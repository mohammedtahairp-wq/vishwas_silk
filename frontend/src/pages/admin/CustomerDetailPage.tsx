...
export function CustomerDetailPage() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><Link to="/admin/customers" className="text-sm text-emerald-600 hover:underline">â† Back to customer search</Link><h1 className="mt-2 text-2xl font-semibold text-gray-900">{customer.name}</h1>{customer.serialNumber && <p className="font-mono text-sm font-semibold text-emerald-600">{customer.serialNumber}</p>}<p className="text-sm text-gray-500">{customer.phone} Â· {customer.address}{customer.villageArea ? ` Â· ${customer.villageArea}` : ""}</p></div>
